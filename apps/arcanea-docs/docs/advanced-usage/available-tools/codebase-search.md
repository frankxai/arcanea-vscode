...
- When Arcanea needs to find code related to specific functionality across your project
...
- When implementing a new feature, Arcanea searches for "authentication middleware" to understand existing patterns before writing new code.
- When debugging an issue, Arcanea searches for "error handling in API calls" to find related error patterns across the codebase.
- When refactoring code, Arcanea searches for "database transaction patterns" to ensure consistency across all database operations.
- When onboarding to a new codebase, Arcanea searches for "configuration loading" to understand how the application bootstraps.
