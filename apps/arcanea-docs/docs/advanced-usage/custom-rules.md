...
Custom rules provide a powerful way to define project-specific and global behaviors and constraints for the Arcanea AI agent. With custom rules, you can ensure consistent formatting, restrict access to sensitive files, enforce coding standards, and customize the AI's behavior for your specific project needs or across all projects.
...
Arcanea supports two types of custom rules:
...
Custom rules are primarily loaded from the **`.arcanea/rules/` directory**. This is the recommended approach for organizing your project-specific rules. Each rule is typically placed in its own Markdown file with a descriptive name:
...
Arcanea provides a built-in interface for managing your custom rules without manually editing files in the `.arcanea/rules/` directories. To access the UI, click on the <Codicon name="law" /> icon in the **bottom right corner** of the Arcanea window.
...
1. **Global rules** from `~/.arcanea/rules/` directory
2. **Project rules** from `.arcanea/rules/` directory
3. **Legacy fallback files** (for backward compatibility):
   - `.roorules`
   - `.clinerules`
   - `.arcanearules` (deprecated)
...
:::note
We strongly recommend keeping your rules in the `.arcanea/rules/` folder as it provides better organization and is the preferred approach for future versions. The folder-based structure allows for more granular rule organization and clearer separation of concerns. The legacy file-based approach is maintained for backward compatibility but may be subject to change in future releases.
:::
...
1. First, it checks for `.arcanea/rules-${mode}/` directory
2. If that doesn't exist or is empty, it falls back to `.arcanearules-${mode}` file (deprecated)
...
<img src="/docs/img/custom-rules/rules-ui.png" alt="Rules tab in Arcanea" width="400" />

The easiest way to create and manage rules is through the built-in UI:

1. Access the rules management interface from the Arcanea panel
...
**For Project Rules:**
1. Create the `.arcanea/rules/` directory if it doesn't already exist
...
Rules will be automatically applied to all future Arcanea interactions. Any new changes will be applied immediately.
...
<img src="/docs/img/custom-rules/custom-rules.png" alt="Arcanea ignores request to read sensitive file" width="600" />
...
When working in team environments, placing `.arcanea/rules/codestyle.md` files under version control allows you to standardize Arcanea's behavior across your entire development team. This ensures consistent code style, documentation practices, and development workflows for everyone on the project.
:::
...
1. **Check rule locations**: Ensure that your rules are located in supported locations:
   - Global rules: `~/.arcanea/rules/` directory
   - Project rules: `.arcanea/rules/` directory
   - Legacy files: `.arcanearules`, `.roorules`, or `.clinerules`
