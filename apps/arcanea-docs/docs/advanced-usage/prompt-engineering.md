...
Prompt engineering is the art of crafting effective instructions for AI models like Arcanea.  Well-written prompts lead to better results, fewer errors, and a more efficient workflow.

## General Principles

*   **Be Clear and Specific:** Clearly state what you want Arcanea to do. Avoid ambiguity.
    *   **Bad:** Fix the code.
...
It's often helpful to guide Arcanea through a "think-then-do" process:

1.  **Analyze:** Ask Arcanea to analyze the current code, identify problems, or plan the approach.
2.  **Plan:**  Have Arcanea outline the steps it will take to complete the task.
3.  **Execute:**  Instruct Arcanea to implement the plan, one step at a time.
4.  **Review:**  Carefully review the results of each step before proceeding.

## Using Custom Instructions

You can provide custom instructions to further tailor Arcanea's behavior. There are two types of custom instructions:
...
*   Adjust Arcanea's tone or personality.

See the [Custom Instructions](/advanced-usage/custom-instructions) section for more details.

## Handling Ambiguity

If your request is ambiguous or lacks sufficient detail, Arcanea might:
...
If Arcanea doesn't produce the desired results, you can provide feedback by:

*   **Rejecting Actions:** Click the "Reject" button when Arcanea proposes an action you don't want.
*   **Providing Explanations:** When rejecting, explain *why* you're rejecting the action.  This helps Arcanea learn from its mistakes.
...
By following these tips, you can write effective prompts that get the most out of Arcanea's capabilities.
