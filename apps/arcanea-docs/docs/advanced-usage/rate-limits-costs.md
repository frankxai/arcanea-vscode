...
Understanding and managing API usage is crucial for a smooth and cost-effective experience with Arcanea. This section explains how to track your token usage, costs, and how to configure rate limits.

## Token Usage

Arcanea interacts with AI models using tokens.  Tokens are essentially pieces of words.  The number of tokens used in a request and response affects both the processing time and the cost.
...
Arcanea automatically calculates the estimated cost of each API request based on the configured model's pricing.  This cost is displayed in the chat history, next to the token usage.
...
To prevent accidental overuse of the API and to help you manage costs, Arcanea allows you to set a rate limit.  The rate limit specifies the minimum time (in seconds) between API requests.
...
If you set the rate limit to 10 seconds, Arcanea will wait at least 10 seconds after one API request completes before sending the next one.
...
*   **Use Custom Instructions:**  Provide custom instructions to guide Arcanea's behavior and reduce the need for lengthy explanations in each prompt.
...
*   **Disable MCP If Not Used:** If you're not using MCP (Model Context Protocol) features, consider [disabling it in the MCP settings](/features/mcp/using-mcp-in-arcanea) to significantly reduce the size of the system prompt and save tokens.

By understanding and managing your API usage, you can use Arcanea effectively and efficiently.