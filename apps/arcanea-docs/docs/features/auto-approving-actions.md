...
> ⚠️ **SECURITY WARNING:** Auto-approve settings bypass confirmation prompts, giving Arcanea direct access to your system. This can result in **data loss, file corruption, or worse**. Command line access is particularly dangerous, as it can potentially execute harmful operations that could damage your system or compromise security. Only enable auto-approval for actions you fully trust.
...
2. Select which actions Arcanea can perform without asking permission
3. Use the master toggle (leftmost checkbox) to quickly enable/disable all permissions

[![Arcanea Task Timeline](https://img.youtube.com/vi/NBccFnYDQ-k/maxresdefault.jpg)](https://youtube.com/shorts/NBccFnYDQ-k?feature=shared)
...
|------------|--------------|------------|
| **Read files and directories** | Lets Arcanea access files without asking | Medium |
| **Edit files** | Lets Arcanea modify files without asking | **High** |
...
| **Use MCP servers** | Lets Arcanea use configured MCP services | Medium-High |
| **Switch modes** | Changes between Arcanea modes automatically | Low |
...
> **Allow Arcanea to automatically perform operations without requiring approval. Enable these settings only if you fully trust the AI and understand the associated security risks.**
...
**Description:** "When enabled, Arcanea will automatically view directory contents and read files without requiring you to click the Approve button."

**Risk level:** Medium

While this setting only allows reading files (not modifying them), it could potentially expose sensitive data. Still recommended as a starting point for most users, but be mindful of what files Arcanea can access.
:::
...
This setting allows Arcanea to modify your files without confirmation. The delay timer is crucial:
...
Allows Arcanea to control a headless browser without confirmation. This can include:
- Opening websites
- Navigating pages
...
This setting automatically retries API calls when they fail. The delay controls how long Arcanea waits before trying again:
...
Allows Arcanea to change between different modes (Code, Architect, etc.) without asking for permission. This primarily affects the AI's behavior rather than system access.
:::
...
Enables Arcanea to create and complete subtasks automatically. This relates to workflow organization rather than system access.
:::
