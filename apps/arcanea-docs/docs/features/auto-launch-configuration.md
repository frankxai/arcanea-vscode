...
Auto-Launch Configuration allows you to automatically start a Arcanea task when opening a workspace, with support for specific profiles and modes. This was originally developed as an internal test feature, but we decided to expose it to users in case anyone finds it useful!
...
When you open a workspace in VS Code, Arcanea automatically checks for a launch configuration JSON file. If found, it:
...
1. Create a `.arcanea` directory in your workspace root (if it doesn't exist)
2. Create a `launchConfig.json` file inside the `.arcanea` directory
...
- **`mode`** (string): The Arcanea mode to use for this task. Available modes:
...
- Auto-launch triggers approximately 500ms after Arcanea extension activation
...
1. Verify file location: `.arcanea/launchConfig.json` in workspace root
