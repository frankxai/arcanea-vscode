...
Checkpoints automatically version your workspace files during Arcanea tasks, enabling non-destructive exploration of AI suggestions and easy recovery from unwanted changes.
...
Access checkpoint settings in Arcanea settings under the "Checkpoints" section:
...
2. Check or uncheck the "Enable automatic checkpoints" checkbox
   ...
   <img src="/docs/img/checkpoints/checkpoints.png" alt="Checkpoint settings in Arcanea configuration" width="500" />

## How Checkpoints Work

Arcanea captures snapshots of your project's state using a shadow Git repository, separate from your main version control system. These snapshots, called checkpoints, automatically record changes throughout your AI-assisted workflow—whenever tasks begin, files change, or commands run.
...
- **Scope**: Checkpoints only capture changes made during active Arcanea tasks
...
When restoration executes, Arcanea:
...
#### .arcaneaignore Behavior

The `.arcaneaignore` file (which controls AI access to files) is separate from checkpoint tracking:
- Files excluded by `.arcaneaignore` but not by `.gitignore` will still be checkpointed
- Changes to AI-inaccessible files can still be restored through checkpoints

This separation is intentional, as `.arcaneaignore` limits which files the AI can access, not which files should be tracked for version history.
