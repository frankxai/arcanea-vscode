...
Codebase Indexing enables semantic code search across your entire project using AI embeddings. Instead of searching for exact text matches, it understands the _meaning_ of your queries, helping Arcanea find relevant code even when you don't know specific function names or file locations.
...
4. **Provides the [`codebase_search`](/advanced-usage/available-tools/codebase-search) tool** to Arcanea for intelligent code discovery
...
- **Enhanced AI Understanding**: Arcanea can better comprehend and work with your codebase
...
1. Open Arcanea settings (<Codicon name="gear" /> icon)
...
- Files matching `.gitignore` and `.arcanea` patterns
...
Once indexed, Arcanea can use the [`codebase_search`](/advanced-usage/available-tools/codebase-search) tool to find relevant code:
...
The tool provides Arcanea with:
