...
Arcanea allows you to create **custom modes** to tailor Arcanea's behavior to specific tasks or workflows. Custom modes can be either **global** (available across all projects) or **project-specific** (defined within a single project).

## Sticky Models for Efficient Workflow

Each mode—including custom ones—features **Sticky Models**. This means Arcanea automatically remembers and selects the last model you used with a particular mode. This lets you assign different preferred models to different tasks without constant reconfiguration, as Arcanea switches between models when you change modes.
...
_Arcanea's interface for creating and managing custom modes._

## What's Included in a Custom Mode?

Custom modes are defined by several key properties. Understanding these concepts will help you tailor Arcanea's behavior effectively.

| UI Field / YAML Property                       | Conceptual Description                                                                                                                                                               |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Slug** (`slug`)                              | A unique internal identifier for the mode. Used by Arcanea to reference the mode, especially for associating mode-specific instruction files.                                      |
| **Name** (`name`)                              | The display name for the mode as it appears in the Arcanea user interface. Should be human-readable and descriptive.                                                               |
...
| **Role Definition** (`roleDefinition`)         | Defines the core identity and expertise of the mode. This text is placed at the beginning of the system prompt and defines Arcanea's personality and behavior when this mode is active. |
...
| **When to Use** (`whenToUse`)                  | _(Optional)_ Provides guidance for Arcanea's automated decision-making, particularly for mode selection and task orchestration. Used by the Orchestrator mode for task coordination.    |
| **Custom Instructions** (`customInstructions`) | _(Optional)_ Specific behavioral guidelines or rules for the mode. Added near the end of the system prompt to further refine Arcanea's behavior.                                        |
...
5. Arcanea packages the mode's configuration and any rules into the YAML file
...
3. Choose the import level:
   - **Project:** Available only in current workspace (saved to `.arcaneamodes` file)
...
### 1. Ask Arcanea! (Recommended)

You can quickly create a basic custom mode by asking Arcanea to do it for you. For example:

```
Create a new mode called "Documentation Writer". It should only be able to read files and write Markdown files.
```

Arcanea will guide you through the process, prompting for necessary information and creating the mode using the preferred YAML format.

### 2. Using the Prompts Tab

1. **Open Prompts Tab:** Click the <Codicon name="notebook" /> icon in the Arcanea top menu bar
...
The interface provides fields for Name, Slug, Description, Save Location, Role Definition, When to Use (optional), Available Tools, and Custom Instructions. After filling these, click the "Create Mode" button. Arcanea will save the new mode in YAML format.

### 3. Manual Configuration (YAML & JSON)

You can directly edit the configuration files to create or modify custom modes. This method offers the most control over all properties. Arcanea now supports both YAML (preferred) and JSON formats.
...
- **Project Modes:** Edit the `.arcaneamodes` file (which can be YAML or JSON) in your project root. Access it via Prompts Tab > <Codicon name="gear" /> (Settings Menu icon next to "Project Prompts") > "Edit Project Modes"
...
- **Usage:** Used internally and in file/directory names for mode-specific rules (e.g., `.arcanea/rules-{slug}/`)
...
- **Purpose:** The display name shown in the Arcanea UI
...
- **Purpose:** Provides guidance for Arcanea's automated decision-making, particularly for mode selection and task orchestration
- **Format:** A string describing ideal scenarios or task types for this mode
- **Usage:** Used by Arcanea for automated decisions and not displayed in the mode selector UI
...
While JSON is still fully supported, new modes created via the UI or by asking Arcanea will default to YAML.
...
### Project Modes (`.arcaneamodes`)

- No automatic startup migration occurs for project-specific files
- Arcanea can read `.arcaneamodes` files in either YAML or JSON format
- When editing through the UI, JSON files will be converted to YAML format
- For manual conversion, you can ask Arcanea to help reformat configurations
...
### Preferred Method: Directory (`.arcanea/rules-{mode-slug}/`)
...
### Fallback Method: Single File (`.arcanearules-{mode-slug}`)

```
.
├── .arcanearules-docs-writer  # Example for mode slug "docs-writer"
└── ... (other project files)
```

**Rules Directory Scope:**

- **Global modes:** Rules are stored in `~/.arcanea/rules-{slug}/`
- **Project modes:** Rules are stored in `{workspace}/.arcanea/rules-{slug}/`
...
1. **Project-level mode configurations** (from `.arcaneamodes` - YAML or JSON)
2. **Global mode configurations** (from `custom_modes.yaml`, then `custom_modes.json` if YAML not found)
3. **Default mode configurations**

**Important:** When modes with the same slug exist in both `.arcaneamodes` and global settings, the `.arcaneamodes` version completely overrides the global one for ALL properties.

## Overriding Default Modes

You can override Arcanea's built-in modes (like 💻 Code, 🪲 Debug, ❓ Ask, 🏗️ Architect, 🪃 Orchestrator) by creating a custom mode with the same slug.
...
:::tip
**Let Arcanea Build Your Regex Patterns**

Instead of writing complex regex manually, ask Arcanea:

```
Create a regex pattern that matches JavaScript files but excludes test files
```

Arcanea will generate the pattern. Remember to adapt it for YAML (usually single backslashes) or JSON (double backslashes).
:::
...
Ready to explore more? Check out the [Show and Tell](https://github.com/Arcanea-Org/arcanea/discussions/categories/show-and-tell) to discover and share custom modes created by the community!
