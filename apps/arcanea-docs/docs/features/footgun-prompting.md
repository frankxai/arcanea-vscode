...
Footgun Prompting, AKA Overriding System Prompt, allows advanced users to completely replace the default system prompt for a specific Arcanea mode. This provides granular control over the AI's behavior but bypasses built-in safeguards.
...
1.  **Override File:** Create a file named `.arcanea/system-prompt-{mode-slug}` in your workspace root (e.g., `.arcanea/system-prompt-code` for the Code mode).
2.  **Content:** The content of this file becomes the new system prompt for that specific mode.
3.  **Activation:** Arcanea automatically detects this file. When present, it replaces most of the standard system prompt sections.
...
You can find the option and instructions within the Arcanea UI:

1.  Click the MODE selector in the bottom-left of the Arcanea text-input box.
...
-   **Intended Audience:** Best suited for users deeply familiar with Arcanea's prompting system and the implications of modifying core instructions.
...
-   **No File, No Override:** If the `.arcanea/system-prompt-{mode-slug}` file doesn't exist, Arcanea uses the standard system prompt generation process for that mode.
-   **Directory Creation:** Arcanea ensures the `.arcanea` directory exists before attempting to read or create the override file.

Use this feature cautiously. While powerful for customization, incorrect implementation can significantly degrade Arcanea's performance and reliability for the affected mode.
