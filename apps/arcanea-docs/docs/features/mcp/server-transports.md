...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Arcanea and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Arcanea) spawns an MCP server as a child process
...
* **Locality**: Runs on the same machine as Arcanea
...
* Single-client scenarios (one Arcanea instance per server)
...
1. The client (Arcanea) connects to the server's SSE endpoint via HTTP GET request
...
* **Remote Access**: Can be hosted on a different machine from Arcanea
...
STDIO servers run locally on the same machine as Arcanea, which has several important implications:
...
* **Execution**: Starts and stops with Arcanea (child process lifecycle)
...
* Start when needed by Arcanea
* Not require network configuration
* Need to be installed alongside Arcanea or via a package manager
...
## Configuring Transports in Arcanea

For detailed information on configuring STDIO and SSE transports in Arcanea, including example configurations, see the [Understanding Transport Types](/features/mcp/using-mcp-in-arcanea#understanding-transport-types) section in the Using MCP in Arcanea guide.