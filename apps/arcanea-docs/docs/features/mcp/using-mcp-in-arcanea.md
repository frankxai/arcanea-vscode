---
title: Using MCP in Arcanea
sidebar_label: Using MCP in Arcanea
---

# Using MCP in Arcanea

Model Context Protocol (MCP) extends Arcanea's capabilities by connecting to external tools and services. This guide covers everything you need to know about using MCP with Arcanea.

<YouTubeEmbed
  url="https://youtu.be/6O9RQoQRX8A"
  caption="Demostrating MCP installation in Arcanea"
/>
...
2.  **Project-level Configuration**: Defined in a `.arcanea/mcp.json` file within your project's root directory. This allows you to set up project-specific servers and share configurations with your team by committing the file to version control. Arcanea automatically detects and loads this file if it exists.
...
You can edit both global and project-level MCP configuration files directly from the Arcanea MCP settings view.

1. Click the <Codicon name="gear" /> icon in the top navigation of the Arcanea pane to open `Settings`.
2. Click the `MCP Servers` tab on the left side
3. Choose the `Installed` servers
4. Click the appropriate button:
    - **`Edit Global MCP`**: Opens the global `mcp_settings.json` file.
    - **`Edit Project MCP`**: Opens the project-specific `.arcanea/mcp.json` file. If this file doesn't exist, Arcanea will create it for you.

  <img src="/docs/img/using-mcp-in-arcanea/mcp-installed-config.png" alt="Edit Global MCP and Edit Project MCP buttons" width="600" />
...
_Example of MCP Server config in Arcanea (STDIO Transport)_
...
2. Press the `Delete` button on the confirmation box
  ...
  <img src="/docs/img/using-mcp-in-arcanea/using-mcp-in-arcanea-5.png" alt="Delete confirmation box" width="400" />
...
<img src="/docs/img/using-mcp-in-arcanea/using-mcp-in-arcanea-6.png" alt="Network Timeout pulldown" width="400" />
...
<img src="/docs/img/using-mcp-in-arcanea/using-mcp-in-arcanea-7.png" alt="Always allow checkbox for MCP tools" width="120" />

When enabled, Arcanea will automatically approve this specific tool without prompting. Note that the global "Use MCP servers" setting takes precedence - if it's disabled, no MCP tools will be auto-approved.

## Finding and Installing MCP Servers

Arcanea does not come with any pre-installed MCP servers. You'll need to find and install them separately.

- **Community Repositories:** Check for community-maintained lists of MCP servers on GitHub
- **Ask Arcanea:** You can ask Arcanea to help you find or even create MCP servers
- **Build Your Own:** Create custom MCP servers using the SDK to extend Arcanea with your own tools

For full SDK documentation, visit the [MCP GitHub repository](https://github.com/modelcontextprotocol/).

## Using MCP Tools in Your Workflow

After configuring an MCP server, Arcanea will automatically detect available tools and resources. To use them:

1. Type your request in the Arcanea chat interface
2. Arcanea will identify when an MCP tool can help with your task
...
- **Permission Errors:** Ensure proper API keys and credentials are configured in your `mcp_settings.json` (for global settings) or `.arcanea/mcp.json` (for project settings).
