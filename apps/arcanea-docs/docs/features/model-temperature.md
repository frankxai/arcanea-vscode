...
## Default Values in Arcanea

Arcanea uses a default temperature of 0.0 for most models, optimizing for maximum determinism and precision in code generation. This applies to OpenAI models, Anthropic models (non-thinking variants), LM Studio models, and most other providers.
...
Some specialized models don't support temperature adjustments at all, in which case Arcanea respects these limitations automatically.
...
1.  **Open the Arcanea Panel:** Click the Arcanea icon (<img src="/docs/img/arcanea-v1.svg" width="12" />) in the VS Code Side Bar
...
5.  **Set Your Value:** Adjust the slider to your preferred value
    ...
    <img src="/docs/img/model-temperature/model-temperature.png" alt="Temperature setting in Arcanea settings panel" width="550" />
    *Temperature slider in Arcanea settings panel*
...
Arcanea implements temperature handling with these considerations:
...
1. **Start with defaults** - Begin with Arcanea's preset values (0.0 for most tasks) as your baseline
...
- Works with all [API providers](/providers/openai) supported by Arcanea
