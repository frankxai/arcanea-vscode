...
Terminal Shell Integration is a key feature that enables Arcanea to execute commands in your terminal and intelligently process their output. This bidirectional communication between the AI and your development environment unlocks powerful automation capabilities.

## What is Shell Integration?

Shell integration is automatically enabled in Arcanea and connects directly to your terminal's command execution lifecycle without requiring any setup from you. This built-in feature allows Arcanea to:
...
When Arcanea needs to perform tasks like installing dependencies, starting a development server, or analyzing build errors, shell integration works behind the scenes to make these interactions smooth and effective.

## Getting Started with Shell Integration

Shell integration is built into Arcanea and works automatically in most cases. If you see "Shell Integration Unavailable" messages or experience issues with command execution, try these solutions:
...
Arcanea provides several settings to fine-tune shell integration. Access these in the Arcanea sidebar under Settings → Terminal.
...
Adds a small pause after running commands to help Arcanea capture all output correctly. This setting can significantly impact shell integration reliability due to VSCode's implementation of terminal integration across different operating systems and shell configurations:
...
Helps PowerShell run the same command multiple times in a row. Enable this if you notice Arcanea can't run identical commands consecutively in PowerShell.

#### Clear ZSH EOL Mark
<img src="/docs/img/shell-integration/clear-zsh-eol-mark.png" alt="Clear ZSH EOL mark checkbox" width="600" />

Prevents ZSH from adding special characters at the end of output lines that can confuse Arcanea when reading terminal results.

#### Oh My Zsh Integration
<img src="/docs/img/shell-integration/oh-my-zsh.png" alt="Enable Oh My Zsh integration checkbox" width="600" />

Makes Arcanea work better with the popular [Oh My Zsh](https://ohmyz.sh/) shell customization framework. Turn this on if you use Oh My Zsh and experience terminal issues.

#### Powerlevel10k Integration
<img src="/docs/img/shell-integration/power10k.png" alt="Enable Powerlevel10k integration checkbox" width="600" />

Improves compatibility if you use the Powerlevel10k theme for ZSH. Turn this on if your fancy terminal prompt causes issues with Arcanea.

#### ZDOTDIR Handling
<img src="/docs/img/shell-integration/zdotdir.png" alt="Enable ZDOTDIR handling checkbox" width="600" />

Helps Arcanea work with custom ZSH configurations without interfering with your personal shell settings and customizations.
...
**Alternative**: Enable the Powerlevel10k Integration setting in Arcanea.
...
1. **Set Terminal Command Delay**:
   - Set the Terminal Command Delay to 50ms in Arcanea settings
...
2. **Roll Back VSCode Version**:
   ...
   - No backup of Arcanea settings needed

3. **WSL-Specific Workaround**:
   - If using WSL, ensure you launch VSCode from within WSL using `code .`

4. **ZSH Users**:
   - Try enabling some or all ZSH-related workarounds in Arcanea settings
...
**Issue**: If text is already typed in the terminal when Arcanea tries to run a command, Arcanea will press Ctrl+C first to clear the line, which can interrupt running processes.

**Workaround**: Make sure your terminal prompt is empty (no partial commands typed) before asking Arcanea to execute terminal commands.

### Multi-line Command Issues

**Issue**: Commands that span multiple lines can confuse Arcanea and may show output from previous commands mixed in with current output.
...
1. **Premature Completion**: PowerShell sometimes tells Arcanea a command is finished before all the output has been shown.
...
1. Check the [Arcanea GitHub Issues](https://github.com/Arcanea-Org/arcanea/issues) to see if others have reported similar problems
2. If not, create a new issue with details about your operating system, VSCode/Cursor version, and the steps you've tried

For additional help, join our [Discord](https://arcanea.ai/discord).
