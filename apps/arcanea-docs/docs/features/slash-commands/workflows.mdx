...
Workflows automate repetitive tasks by defining step-by-step instructions for Arcanea to execute. Invoke any workflow by typing `/[workflow-name.md]` in the chat.

<img src="/docs/img/slash-commands/workflows.png" alt="Workflows tab in Arcanea" width="600" />

## Creating Workflows

Workflows are markdown files stored in `.arcanea/workflows/`:
- **Global workflows**: `~/.arcanea/workflows/` (available in all projects)
- **Project workflows**: `[project]/.arcanea/workflows/` (project-specific)
...
Create a file called `submit-pr.md` in your `.arcanea/workflows` directory:
...
Now you can trigger this workflow by typing `/submit-pr.md` in the chat. Arcanea will:
