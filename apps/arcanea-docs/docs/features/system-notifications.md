...
Arcanea uses system notifications to inform you about:
...
Arcanea's system notifications work on all major operating systems with different underlying technologies:
...
No additional setup required. Arcanea uses macOS's built-in command to display notifications.
...
**How it works:** Arcanea first attempts to use `terminal-notifier` and automatically falls back to AppleScript if it's not installed.
