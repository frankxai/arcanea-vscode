...
- When Arcanea needs to make precise changes to existing code without rewriting entire files.
...
- Validates file access against `.arcaneaignore` rules.
...
2.  **ArcaneaIgnore Check**: Validates if the target file path is allowed by `.arcaneaignore` rules.
...
Arcanea uses this strategy for applying diffs:
