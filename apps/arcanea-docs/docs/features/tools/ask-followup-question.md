...
This tool creates a conversational interface between Arcanea and the user, allowing for gathering clarification, additional details, or user preferences when facing ambiguities or decision points. Each question can include suggested responses to streamline the interaction.

## When is it used?

- When critical information is missing from the original request
- When Arcanea needs to choose between multiple valid implementation approaches
- When technical details or preferences are required to proceed
- When Arcanea encounters ambiguities that need resolution
...
1. **Information Gap Recognition**: Arcanea identifies missing information needed to proceed
2. **Specific Question Creation**: Arcanea formulates a clear, targeted question
3. **Suggestion Development**: Arcanea creates relevant suggested answers (optional but recommended)
...
10. **Task Continuation**: Arcanea proceeds with the task using the new information

## Examples When Used

- When developing a web application, Arcanea might ask about preferred styling frameworks (Bootstrap, Tailwind, custom CSS)
- When creating an API, Arcanea might ask about authentication methods (JWT, OAuth, API keys)
- When refactoring code, Arcanea might ask about prioritizing performance vs. readability
- When setting up a database, Arcanea might ask about specific schema design preferences
- When creating a custom feature, Arcanea might ask about specific behavior expectations
- When troubleshooting errors, Arcanea might ask about specific environment details
