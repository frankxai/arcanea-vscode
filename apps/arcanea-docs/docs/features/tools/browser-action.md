...
The `browser_action` tool enables web automation and interaction via a Puppeteer-controlled browser. It allows Arcanea to launch browsers, navigate to websites, click elements, type text, and scroll pages with visual feedback through screenshots.
...
This tool creates an automated browser session that Arcanea can control to navigate websites, interact with elements, and perform tasks that require browser automation. Each action provides a screenshot of the current state, enabling visual verification of the process.

## When is it used?

- When Arcanea needs to interact with web applications or websites
...
- Browser sessions are temporary and not persistent across Arcanea restarts
...
- When creating a web form submission process, Arcanea launches a browser, navigates to the form, fills out fields with the `type` action, and clicks submit.
- When testing a responsive website, Arcanea navigates to the site and uses scroll actions to examine different sections.
- When capturing screenshots of a web application, Arcanea navigates through different pages and takes screenshots at each step.
- When demonstrating an e-commerce checkout flow, Arcanea simulates the entire process from product selection to payment confirmation.
