...
The `execute_command` tool runs CLI commands on the user's system. It allows Arcanea to perform system operations, install dependencies, build projects, start servers, and execute other terminal-based tasks needed to accomplish user objectives.
...
- Integrates with ArcaneaIgnore system for file access control
- Handles terminal escape sequences for clean output

## Limitations

- Command access may be restricted by ArcaneaIgnore rules and security validations
...
1. **Command Validation and Security Checks**:
   ...
   - Checks against ArcaneaIgnore rules for file access permissions
...
2. **Second Priority: Security Validation**
   ...
   - Commands are checked against ArcaneaIgnore rules for file access control
...
- When setting up a new project, Arcanea runs initialization commands like `npm init -y` followed by installing dependencies.
- When building a web application, Arcanea executes build commands like `npm run build` to compile assets.
- When deploying code, Arcanea runs git commands to commit and push changes to a repository.
- When troubleshooting, Arcanea executes diagnostic commands to gather system information.
- When starting a development server, Arcanea launches the appropriate server command (e.g., `npm start`).
- When running tests, Arcanea executes the test runner command for the project's testing framework.
