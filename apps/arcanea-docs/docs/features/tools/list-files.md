...
The `list_files` tool displays the files and directories within a specified location. It helps Arcanea understand your project structure and navigate your codebase effectively.
...
- When Arcanea needs to understand your project structure
- When Arcanea explores what files are available before reading specific ones
- When Arcanea maps a codebase to better understand its organization
- Before using more targeted tools like `read_file` or `search_files`
- When Arcanea needs to check for specific file types (like configuration files) across a project
...
- Marks files ignored by `.arcaneaignore` with a lock symbol (🔒) when `showArcaneaIgnoredFiles` is enabled
...
5. **Result Filtering**:
   ...
   - Respects `.gitignore` rules when in recursive mode
   - Handles `.arcaneaignore` patterns, either hiding files or marking them with a lock symbol
6. **Formatting**:
   - Marks directories with a trailing slash (`/`)
   - Sorts results to show directories before their contents for logical hierarchy
   - Marks ignored files with a lock symbol (🔒) when `showArcaneaIgnored` is enabled
...
- Files ignored by `.arcaneaignore` are marked with a lock symbol (🔒) when `showArcaneaIgnored` is enabled
...
When `.arcaneaignore` files are used and `showArcaneaIgnored` is enabled:
```
src/
src/components/
src/components/Button.tsx
...
- When starting a new task, Arcanea may list the project files to understand its structure before diving into specific code.
- When asked to find specific types of files (like all JavaScript files), Arcanea first lists directories to know where to look.
- When providing recommendations for code organization, Arcanea examines the current project structure first.
- When setting up a new feature, Arcanea lists related directories to understand the project conventions.
