...
The `search_files` tool performs regex searches across multiple files in your project. It helps Arcanea locate specific code patterns, text, or other content throughout your codebase with contextual results.
...
- When Arcanea needs to find where specific functions or variables are used
- When Arcanea helps with refactoring and needs to understand usage patterns
- When Arcanea needs to locate all instances of a particular code pattern
- When Arcanea searches for text across multiple files with filtering capabilities
...
- When asked to refactor a function, Arcanea first searches for all places the function is used to ensure comprehensive changes.
- When investigating bugs, Arcanea searches for similar patterns to identify related issues across the codebase.
- When addressing technical debt, Arcanea locates all TODO comments across the project.
- When analyzing dependencies, Arcanea finds all imports of a particular module.
