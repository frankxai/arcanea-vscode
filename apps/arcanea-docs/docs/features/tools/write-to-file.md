...
- When Arcanea needs to create a new file from scratch
- When Arcanea needs to completely rewrite an existing file
...
- Access Control: Validates against `.arcaneaignore` restrictions before making changes
...
1. **Parameter Validation**: Validates the required parameters and permissions
   ...
   - Validates the file is allowed (not restricted by `.arcaneaignore`)
...
- When creating a new project, Arcanea generates multiple files but lets you review each before committing changes.
- When setting up configuration files, Arcanea shows the proposed configuration in a diff view for approval.
- When generating documentation, Arcanea creates markdown files but lets you make final adjustments in the diff view.
- When developing a prototype, Arcanea shows complete source files in a diff view where you can fine-tune before saving.
