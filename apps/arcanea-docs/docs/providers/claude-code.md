...
# Using Claude Code With Arcanea

Claude Code is Anthropic's official CLI that provides direct access to Claude models from your terminal. Using Claude Code with Arcanea lets you leverage your existing CLI setup without needing separate API keys.
...
If this environment variable is set on your system, the `claude` tool may use it for authentication instead of the interactive `/login` method. When Arcanea executes the tool, it will accurately reflect that an API key is being used, as this is the underlying behavior of the `claude` CLI itself.
:::
...
Arcanea supports the following Claude models through Claude Code:
...
- **Enhance Prompt Feature:** Full compatibility with Arcanea's Enhance Prompt feature, allowing you to automatically improve and refine your prompts before sending them to Claude.
