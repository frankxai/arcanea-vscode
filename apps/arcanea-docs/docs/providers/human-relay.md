...
The Human Relay provider allows you to use Arcanea with web-based AI models like ChatGPT or Claude without needing an API key. Instead, it relies on you to manually relay messages between Arcanea and the AI's web interface.

## How it Works

1.  **Select Human Relay**: Choose "Human Relay" as your API provider in Arcanea's settings. No API key is required.
2.  **Initiate a Request**: Start a chat or task with Arcanea as usual.
...
6.  **Paste Back to Arcanea**: Return to the dialog box in VS Code, paste the AI's response into the designated field, and click "Confirm".
7.  **Continue**: Arcanea will process the response as if it came directly from an API.
