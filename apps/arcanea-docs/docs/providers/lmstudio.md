...
# Using LM Studio With Arcanea

Arcanea supports running models locally using LM Studio.  LM Studio provides a user-friendly interface for downloading, configuring, and running local language models.  It also includes a built-in local inference server that emulates the OpenAI API, making it easy to integrate with Arcanea.
...
2.  **Download a Model:**  Use the LM Studio interface to search for and download a model.  Some recommended models include:
    ...
    * Any other model that is supported by Arcanea, or for which you can set the context window.
...
4.  **(Optional) Base URL:**  By default, Arcanea will connect to LM Studio at `http://localhost:1234`.  If you've configured LM Studio to use a different address or port, enter the full URL here.
...
*   **Local Server:**  The LM Studio local server must be running for Arcanea to connect to it.
