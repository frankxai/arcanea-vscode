...
- **Usage Tracking:** You can set custom limits for each provider based on the number of tokens or requests per minute, hour, or day. Arcanea tracks the usage for each provider against these limits.
...
Before configuring this provider, you must have at least one other API provider already configured as a separate profile in Arcanea. This provider is only useful if there are other profiles for it to manage.

## Configuration in Arcanea

1.  **Open Arcanea Settings:** Click the gear icon (<Codicon name="gear" />) in the Arcanea panel.
...
<img src="/docs/img/providers/virtualQuotaSelectDropdown.png" alt="virtuaQuotaFallback dropdown selection in Arcanea settings" width="600" />
...
<img src="/docs/img/providers/virtualQuotaFullConfig.png" alt="virtuaQuotaFallback configuration in Arcanea settings" width="600" />
...
**Result:**
Arcanea will send all requests to Chutes AI. Once your usage exceeds 5,000 tokens within an hour, it will automatically switch to your OpenAI account. The system will switch back to Chutes AI in the next hour when its quota window has reset.
