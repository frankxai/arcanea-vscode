...
- 当 Arcanea 实现新功能时，搜索 "身份验证中间件" 以在编写新代码之前了解现有模式。
- 当调试问题时，Arcanea 搜索 "API 调用中的错误处理" 以查找代码库中相关的错误模式。
- 当重构代码时，Arcanea 搜索 "数据库事务模式" 以确保所有数据库操作的一致性。
- 当加入新代码库时，Arcanea 搜索 "配置加载" 以了解应用程序如何引导。
