...
Prompt engineering 是为 AI 模型（如 Arcanea）编写有效指令的艺术。精心编写的提示可以带来更好的结果、更少的错误和更高的工作效率。

## 通用原则

*   **清晰且具体：** 清楚地说明你希望 Arcanea 做什么。避免歧义。
    *   **不好：** 修复代码。
...
引导 Arcanea 通过“先思考后执行”的过程通常很有帮助：

1.  **分析：** 让 Arcanea 分析当前代码，识别问题或规划方法。
2.  **计划：** 让 Arcanea 概述完成任务的步骤。
3.  **执行：** 指示 Arcanea 一步一步地执行计划。
4.  **审查：** 在继续之前仔细审查每一步的结果。

## 使用自定义指令

你可以提供自定义指令来进一步调整 Arcanea 的行为。有两种类型的自定义指令：
...
如果你的请求含糊不清或缺乏足够的细节，Arcanea 可能会：
...
如果 Arcanea 没有产生预期的结果，你可以通过以下方式提供反馈：

*   **拒绝操作：** 当 Arcanea 提出你不想要的操作时，点击“拒绝”按钮。
*   **提供解释：** 拒绝时，解释*为什么*要拒绝该操作。这有助于 Arcanea 从错误中学习。
...
通过遵循这些技巧，你可以编写有效的提示，充分利用 Arcanea 的能力。