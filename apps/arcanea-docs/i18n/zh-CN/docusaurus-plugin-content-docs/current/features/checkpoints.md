...
检查点在 Arcanea 任务期间自动对您的工作区文件进行版本控制，从而实现对 AI 建议的非破坏性探索，并轻松从不需要的更改中恢复。
...
在 Arcanea 设置的“检查点”部分访问检查点设置：

1.  单击齿轮图标 <Codicon name="gear" /> → Checkpoints 打开设置
2.  选中或取消选中“启用自动检查点”复选框

    <img src="/docs/img/checkpoints/checkpoints.png" alt="Arcanea 配置中的检查点设置" width="500" />

## 检查点工作原理

Arcanea 使用一个影子 Git 仓库（独立于您的主版本控制系统）捕获项目状态的快照。这些快照称为检查点，会在您的 AI 辅助工作流中自动记录更改——无论任务何时开始、文件何时更改或命令何时运行。
...
- **范围**：检查点仅捕获在活动 Arcanea 任务期间所做的更改
...
`.arcaneaignore` 文件（控制 AI 对文件的访问）与检查点跟踪是分开的：
- 被 `.arcaneaignore` 排除但未被 `.gitignore` 排除的文件仍将被检查点
- 对 AI 无法访问的文件的更改仍可通过检查点恢复

这种分离是故意的，因为 `.arcaneaignore` 限制了 AI 可以访问的文件，而不是应该跟踪版本历史的文件。
