...
代码库索引通过 AI 嵌入实现对整个项目的语义代码搜索。它不是搜索精确的文本匹配，而是理解您查询的*含义*，帮助 Arcanea 找到相关代码，即使您不知道特定的函数名称或文件位置。
...
4.  **为 Arcanea 提供 [`codebase_search`](/advanced-usage/available-tools/codebase-search) 工具**以进行智能代码发现
...
- **增强 AI 理解**：Arcanea 可以更好地理解和使用您的代码库
...
1.  打开 Arcanea 设置（<Codicon name="gear" /> 图标）
2.  导航到 **代码库索引** 部分
3.  使用切换开关启用 **“启用代码库索引”**
4.  配置您的嵌入提供商：
...
索引后，Arcanea 可以使用 [`codebase_search`](/advanced-usage/available-tools/codebase-search) 工具查找相关代码：
