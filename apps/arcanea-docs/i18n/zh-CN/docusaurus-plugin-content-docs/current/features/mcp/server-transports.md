...
模型上下文协议（MCP）支持两种主要的传输机制用于Arcanea与MCP服务器之间的通信：标准输入/输出（STDIO）和服务器发送事件（SSE）。每种机制都有其独特的特点、优势和适用场景。
...
1. 客户端（Arcanea）通过HTTP GET请求连接到服务器的SSE端点
...
STDIO服务器与Arcanea在同一台机器上运行，这有几个重要影响：
...
有关在Arcanea中配置STDIO和SSE传输的详细信息，包括示例配置，请参见[理解传输类型](/features/mcp/using-mcp-in-arcanea#understanding-transport-types)部分。