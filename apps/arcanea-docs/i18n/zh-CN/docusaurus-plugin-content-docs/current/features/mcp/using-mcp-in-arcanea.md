---
title: 在Arcanea中使用MCP
sidebar_label: 在Arcanea中使用MCP
---

# 在Arcanea中使用MCP

模型上下文协议（MCP）通过连接外部工具和服务扩展了Arcanea的功能。本指南涵盖了在Arcanea中使用MCP所需了解的所有内容。

<YouTubeEmbed
  url="https://youtu.be/6O9RQoQRX8A"
  caption="演示Arcanea中的MCP安装"
/>
...
2. **项目级配置**：在项目根目录的`.arcanea/mcp.json`文件中定义。这允许你设置项目特定的服务器，并通过将文件提交到版本控制与团队共享配置。如果存在，Arcanea会自动检测并加载此文件。

**优先级**：如果服务器名称同时存在于全局和项目配置中，则**项目级配置优先**。

### 编辑MCP设置文件

你可以直接从Arcanea MCP设置视图编辑全局和项目级MCP配置文件：

1.  单击 Arcanea 窗格顶部导航栏中的 <Codicon name="gear" /> 图标以打开 `Settings`。
2.  单击左侧的 `MCP Servers` 选项卡
3.  选择 `Installed` 服务器
3.  单击相应的按钮：
    *   **`Edit Global MCP`**：打开全局 `mcp_settings.json` 文件。
    *   **`Edit Project MCP`**：打开项目特定的 `.arcanea/mcp.json` 文件。如果此文件不存在，Arcanea 将为您创建它。

  <img src="/docs/img/using-mcp-in-arcanea/mcp-installed-config.png" alt="编辑全局 MCP 和编辑项目 MCP 按钮" width="600" />
...
2. 在确认框中点击`删除`按钮
   ...
   <img src="/docs/img/using-mcp-in-arcanea/using-mcp-in-arcanea-5.png" alt="删除确认框" width="400" />
...
<img src="/docs/img/using-mcp-in-arcanea/using-mcp-in-arcanea-6.png" alt="网络超时下拉菜单" width="400" />
...
<img src="/docs/img/using-mcp-in-arcanea/using-mcp-in-arcanea-7.png" alt="MCP工具的始终允许复选框" width="120" />

启用后，Arcanea将自动批准此特定工具而无需提示。请注意，全局"使用MCP服务器"设置优先 - 如果它被禁用，则不会自动批准任何MCP工具。

## 查找和安装MCP服务器

Arcanea不附带任何预安装的MCP服务器。你需要单独查找并安装它们。

* **社区仓库**：在GitHub上查看社区维护的MCP服务器列表
* **询问Arcanea**：你可以请Arcanea帮助你查找甚至创建MCP服务器（当"[启用MCP服务器创建](#enabling-or-disabling-mcp-server-creation)"启用时）
* **自行构建**：使用SDK创建自定义MCP服务器，使用你自己的工具扩展Arcanea

有关完整的SDK文档，请访问[MCP GitHub仓库](https://github.com/modelcontextprotocol/)。

## 在工作流中使用MCP工具

配置MCP服务器后，Arcanea将自动检测可用的工具和资源。要使用它们：

1. 在Arcanea聊天界面中输入你的请求
2. Arcanea将识别何时MCP工具可以帮助完成任务
...
* **权限错误**：确保在`mcp_settings.json`（用于全局设置）或`.arcanea/mcp.json`（用于项目设置）中配置了正确的API密钥和凭据。
