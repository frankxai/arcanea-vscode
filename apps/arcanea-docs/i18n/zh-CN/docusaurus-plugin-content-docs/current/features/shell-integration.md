...
终端 Shell 集成是一项关键功能，它使 Arcanea 能够在您的终端中执行命令并智能地处理其输出。AI 与您的开发环境之间的这种双向通信解锁了强大的自动化功能。

## 什么是 Shell 集成？

Shell 集成在 Arcanea 中自动启用，并直接连接到您的终端命令执行生命周期，无需您进行任何设置。此内置功能允许 Arcanea：
...
当 Arcanea 需要执行安装依赖项、启动开发服务器或分析构建错误等任务时，shell 集成会在后台工作，使这些交互顺畅有效。

## Shell 集成入门

Shell 集成内置于 Arcanea 中，在大多数情况下会自动工作。如果您看到“Shell 集成不可用”消息或遇到命令执行问题，请尝试以下解决方案：
...
Arcanea 提供了几个设置来微调 shell 集成。在 Arcanea 侧边栏的“设置”→“终端”下访问这些设置。
...
在运行命令后添加一个短暂的暂停，以帮助 Arcanea 正确捕获所有输出。由于 VSCode 在不同操作系统和 shell 配置中实现终端集成的方式，此设置可能会显著影响 shell 集成的可靠性：
...
**替代方案**：在 Arcanea 设置中启用 Powerlevel10k 集成设置。
...
**解决方案**：
1. **设置终端命令延迟**：
    - 在 Arcanea 设置中将“终端命令延迟”设置为 50 毫秒。
    - 更改此设置后，重启所有终端。
...
4. **ZSH 用户**：
    - 尝试在 Arcanea 设置中启用部分或全部 ZSH 相关的解决方法。
...
**问题**：当 Arcanea 尝试运行命令时，如果终端中已经输入了文本，Arcanea 会先按 Ctrl+C 来清空当前行，这可能会中断正在运行的进程。

**解决方法**：在要求 Arcanea 执行终端命令之前，请确保您的终端提示符是空的（没有输入任何部分命令）。

### 多行命令问题

**问题**：跨多行的命令可能会让 Arcanea 混淆，并可能将先前命令的输出与当前输出混合在一起。

**解决方法**：不要使用多行命令，而是使用 `&&` 进行命令链接，将所有内容保持在一行（例如，使用 `echo a && echo b`，而不是分行输入每个命令）。

### PowerShell 特定问题

1.  **过早完成**：PowerShell 有时会在所有输出显示完毕前就通知 Arcanea 命令已完成。
...
1.  查看 [Arcanea GitHub Issues](https://github.com/Arcanea-Org/arcanea/issues) 页面，看是否有人报告了类似的问题。
2.  如果没有，请创建一个新 issue，并提供有关您的操作系统、VSCode/Cursor 版本以及您已尝试过的步骤的详细信息。

如需更多帮助，请加入我们的 [Discord](https://arcanea.ai/discord)。如需其他帮助，请加入我们的 [Discord](https://arcanea.ai/discord)。
