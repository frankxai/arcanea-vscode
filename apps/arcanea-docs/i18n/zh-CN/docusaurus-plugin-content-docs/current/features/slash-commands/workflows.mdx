...
Workflows 通过为 Arcanea 定义分步指令来自动化重复性任务。通过在聊天中键入 `/[workflow-name.md]` 来调用任何工作流。

<img src="/docs/img/slash-commands/workflows.png" alt="Arcanea 中的工作流选项卡" width="600" />

## 创建工作流

工作流是存储在 `.arcanea/workflows/` 中的 markdown 文件：
- **全局工作流**：`~/.arcanea/workflows/`（在所有项目中可用）
- **项目工作流**：`[project]/.arcanea/workflows/`（项目特定）
...
在您的 `.arcanea/workflows` 目录中创建一个名为 `submit-pr.md` 的文件：
...
现在您可以通过在聊天中键入 `/submit-pr.md` 来触发此工作流。Arcanea 将：
