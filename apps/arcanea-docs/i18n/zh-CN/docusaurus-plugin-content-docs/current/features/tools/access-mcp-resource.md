...
`access_mcp_resource` 工具用于从连接的 Model Context Protocol (MCP) 服务器中检索资源数据。它允许 Arcanea 访问文件、API 响应、文档或系统信息，为任务提供额外的上下文。
...
- 当 Arcanea 需要从外部系统获取额外上下文时
- 当 Arcanea 需要从专门的 MCP 服务器访问领域特定数据时
- 当 Arcanea 需要检索由 MCP 服务器托管的参考文档时
- 当 Arcanea 需要通过 MCP 从外部 API 集成实时数据时
...
- 在帮助开发 API 时，Arcanea 从 MCP 资源中检索端点规范以确保正确实现。
- 在协助数据可视化时，Arcanea 从连接的 MCP 服务器访问当前数据样本。
- 在特定领域工作时，Arcanea 检索技术文档以提供准确的指导。
- 在生成行业特定代码时，Arcanea 参考文档中的合规要求。
