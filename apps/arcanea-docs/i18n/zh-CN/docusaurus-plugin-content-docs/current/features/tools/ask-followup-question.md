...
该工具在 Arcanea 和用户之间创建了一个对话界面，当遇到模糊不清或决策点时，可以用于收集澄清信息、额外细节或用户偏好。每个问题可以包含建议的响应，以简化交互。

## 使用场景

- 当原始请求中缺少关键信息时
- 当 Arcanea 需要在多个有效的实现方法之间做出选择时
- 当需要技术细节或偏好才能继续时
- 当 Arcanea 遇到需要解决的模糊不清的情况时
- 当额外的上下文会显著提高解决方案质量时
...
1. **信息差距识别**: Arcanea 识别出继续所需缺少的信息
2. **具体问题创建**: Arcanea 形成一个明确的、有针对性的问题
3. **建议开发**: Arcanea 创建相关的建议答案（可选但推荐）
...
9. **上下文集成**: 响应被添加到对话历史中
10. **任务继续**: Arcanea 使用新信息继续任务

## 使用示例

- 在开发 Web 应用程序时，Arcanea 可能会询问首选的样式框架（Bootstrap、Tailwind、自定义 CSS）
- 在创建 API 时，Arcanea 可能会询问认证方法（JWT、OAuth、API 密钥）
- 在重构代码时，Arcanea 可能会询问是优先考虑性能还是可读性
- 在设置数据库时，Arcanea 可能会询问特定的模式设计偏好
- 在创建自定义功能时，Arcanea 可能会询问特定的行为期望
- 在排查错误时，Arcanea 可能会询问特定的环境细节
