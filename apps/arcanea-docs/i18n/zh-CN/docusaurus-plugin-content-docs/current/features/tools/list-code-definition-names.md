...
`list_code_definition_names` 工具通过列出指定目录顶层源代码文件中的代码定义，为您的代码库提供结构化概览。它通过显示行号和定义片段，帮助 Arcanea 理解代码架构。  
...
- 当 Arcanea 需要快速理解您的代码库架构时  
- 当 Arcanea 需要定位多个文件中的重要代码结构时  
- 在计划重构或扩展现有代码时  
- 在使用其他工具深入研究实现细节之前  
- 在识别代码库不同部分之间的关系时  
...
- 当开始新任务时，Arcanea 首先列出关键代码定义以了解项目的整体结构。  
- 当计划重构工作时，Arcanea 使用此工具识别可能受影响的类和函数。  
- 在探索不熟悉的代码库时，Arcanea 在深入研究实现细节之前映射重要代码结构。  
- 当添加新功能时，Arcanea 识别现有模式和相关的代码定义以保持一致性。  
- 当排查错误时，Arcanea 映射代码库结构以定位问题的潜在来源。  
- 当计划架构更改时，Arcanea 识别所有受影响的组件。  
