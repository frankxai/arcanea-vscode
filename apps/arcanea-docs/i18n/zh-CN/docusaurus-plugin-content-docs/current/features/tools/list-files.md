...
`list_files` 工具显示指定位置中的文件和目录。它帮助 Arcanea 理解您的项目结构并有效地导航代码库。  
...
- 当 Arcanea 需要理解您的项目结构时  
- 当 Arcanea 在读取特定文件之前探索可用文件时  
- 当 Arcanea 映射代码库以更好地理解其组织方式时  
- 在使用更针对性的工具（如 `read_file` 或 `search_files`）之前  
- 当 Arcanea 需要检查项目中特定类型的文件（如配置文件）时  
...
- 当启用 `showArcaneaIgnoredFiles` 时，用锁符号（🔒）标记被 `.arcaneaignore` 忽略的文件  
- 通过逐级目录遍历优化性能  
...
5. **结果过滤**：  
   ...
   - 在递归模式下，遵循 `.gitignore` 规则  
   - 处理 `.arcaneaignore` 规则，要么隐藏文件，要么用锁符号标记  
6. **格式化**：  
   - 用斜杠（`/`）标记目录  
   - 排序结果，先显示目录及其内容，保持逻辑层次结构  
   - 当启用 `showArcaneaIgnored` 时，用锁符号（🔒）标记被忽略的文件  
...
- 每个文件路径显示在其自己的行上  
- 目录用斜杠（`/`）标记  
- 当启用 `showArcaneaIgnored` 时，被 `.arcaneaignore` 忽略的文件用锁符号（🔒）标记  
- 结果按逻辑排序，先显示目录及其内容  
...
当使用 `.arcaneaignore` 文件并启用 `showArcaneaIgnored` 时：  
```  
src/  
src/components/  
src/components/Button.tsx  
...
- 当开始新任务时，Arcanea 可能会列出项目文件以了解其结构，然后再深入研究特定代码。  
- 当被要求查找特定类型的文件（如所有 JavaScript 文件）时，Arcanea 首先列出目录以知道在哪里查找。  
- 当提供代码组织建议时，Arcanea 首先检查当前项目结构。  
- 当设置新功能时，Arcanea 列出相关目录以了解项目约定。  
