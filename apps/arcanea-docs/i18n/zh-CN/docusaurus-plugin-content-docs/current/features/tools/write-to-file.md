...
- 当 Arcanea 需要从头创建新文件时
- 当 Arcanea 需要完全重写现有文件时
- 当为新项目创建多个文件时
- 当生成配置文件、文档或源代码时
- 当需要在应用变更之前查看变更时
...
- 创建新项目时，Arcanea 会生成多个文件，但会让您在提交更改前查看每个文件。  
- 设置配置文件时，Arcanea 会在差异视图中显示建议的配置以供批准。  
- 生成文档时，Arcanea 会创建 Markdown 文件，但允许您在差异视图中进行最终调整。  
- 开发原型时，Arcanea 会在差异视图中显示完整的源文件，您可以在保存前进行微调。  
