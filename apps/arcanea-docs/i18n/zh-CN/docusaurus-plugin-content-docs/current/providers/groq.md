...
Groq 通过其高性能基础设施为各种 AI 模型提供超快速推理。Arcanea 支持通过 Groq API 访问模型。

**网站：** [https://groq.com/](https://groq.com/)

## 获取 API 密钥

要将 Groq 与 Arcanea 一起使用，您需要从 [GroqCloud 控制台](https://console.groq.com/)获取 API 密钥。注册或登录后，导航到仪表板的 API 密钥部分以创建和复制您的密钥。

## 支持的模型

Arcanea 将尝试从 Groq API 获取可用模型列表。通过 Groq 通常可用的模型包括：
...
*   **Token 限制：** 某些模型具有特定的 `max_tokens` 限制，Arcanea 会自动处理（例如，`moonshotai/kimi-k2-instruct` 模型）。
*   **成本效益：** 高性能推理具有竞争力的定价。
*   **模型选择：** 根据您的特定需求选择模型 - 对于复杂推理任务，选择 `llama3-70b-8192` 等大型模型，或者对于更快、更简单的操作，选择 `llama3-8b-8192` 等小型模型。

## 支持的模型

Arcanea 通过 Groq 支持以下模型：
