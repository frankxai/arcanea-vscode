...
人工中继提供商允许您在不需要 API 密钥的情况下，使用基于 Web 的 AI 模型（如 ChatGPT 或 Claude）与 Arcanea 配合使用。相反，它依赖于您在 Arcanea 和 AI 的 Web 界面之间手动传递消息。

## 工作原理

1. **选择人工中继：** 在 Arcanea 的设置中选择 "人工中继" 作为您的 API 提供商。无需 API 密钥。
2. **发起请求：** 像往常一样开始与 Arcanea 的聊天或任务。
...
6. **粘贴回 Arcanea：** 返回 VS Code 中的对话框，将 AI 的回复粘贴到指定字段中，然后点击“确认”。
7. **继续：** Arcanea 会像处理直接来自 API 的响应一样处理该回复。
