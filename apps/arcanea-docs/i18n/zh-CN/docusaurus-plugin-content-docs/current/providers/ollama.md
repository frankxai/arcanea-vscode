...
Arcanea 支持通过 Ollama 在本地运行模型。这提供了隐私保护、离线访问能力，并可能降低成本，但需要更多设置且依赖高性能计算机。  
...
*   必须至少有 32k 的上下文窗口（这是 Arcanea 的要求）。
*   列出为支持工具。
*   参数数量在 7b 到 24b 范围内。
*   优先选择流行模型。
*   优先选择较新的模型。
...
以下模型看起来是合理的选择，但发现与 Arcanea 的默认配置**不**兼容：
...
2.  **下载模型：** 下载模型后，您可以离线使用 Arcanea 和该模型。要下载模型，请打开终端并运行：
...
4.  **配置 Arcanea：**
    *   打开 Arcanea 侧边栏（<img src="/docs/img/arcanea-v1.svg" width="12" /> 图标）。
