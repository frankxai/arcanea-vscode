...
# 使用 OpenAI 兼容的提供商与 Arcanea

Arcanea 支持多种提供 OpenAI API 标准兼容的 AI 模型提供商。这意味着你可以使用 *除了* OpenAI 之外的其他提供商的模型，同时仍然使用熟悉的 API 接口。这包括以下提供商：
...
你可以在 Arcanea 设置面板中找到这些设置（点击 <Codicon name="gear" /> 图标）：
...
Arcanea 支持在基本 URL 字段中输入完整端点 URL，为提供商配置提供更大的灵活性：
...
虽然此提供商类型允许连接到各种端点，但如果你直接连接到官方 OpenAI API（或完全镜像它的端点），Arcanea 会根据其源代码中的 `openAiNativeModels` 定义识别以下模型 ID：
...
通过使用 OpenAI 兼容的提供商，你可以利用 Arcanea 的灵活性与更广泛的 AI 模型。请始终参考你提供商的文档以获取最准确和最新的信息。
