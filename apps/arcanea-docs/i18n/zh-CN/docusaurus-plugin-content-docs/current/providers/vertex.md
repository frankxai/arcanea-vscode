...
# 在 Arcanea 中使用 GCP Vertex AI

Arcanea 支持通过 Google Cloud Platform 的 Vertex AI 访问模型，这是一个托管的机器学习平台，提供对多种基础模型的访问，包括 Anthropic 的 Claude 系列。
...
*   **应用程序默认凭据 (ADC):** Arcanea 使用应用程序默认凭据来与 Vertex AI 进行身份验证。最简单的设置方法是：
...
Arcanea 通过 Vertex AI 支持以下模型（基于源代码）：
