...
import { getGitSha, copyPaths, copyLocales, copyWasms, generatePackageJson } from "@arcanea/build"
...
async function main() {
	...
	const buildOptions = {
		...
		platform: "node",
		define: {
			"process.env.PKG_NAME": '"arcanea-nightly"',
			"process.env.PKG_VERSION": `"${overrideJson.version}"`,
			"process.env.PKG_OUTPUT_CHANNEL": '"Arcanea-Code-Nightly"',
			...
			setup(build) {
				build.onEnd(() => {
					...
					const generatedPackageJson = generatePackageJson({
						packageJson,
						overrideJson,
						substitution: ["arcanea", "arcanea-nightly"],
					})
