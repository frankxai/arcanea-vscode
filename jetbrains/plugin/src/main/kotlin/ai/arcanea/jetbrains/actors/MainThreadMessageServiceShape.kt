...
class MainThreadMessageService : MainThreadMessageServiceShape {
    ...
    private fun showModalMessage(
        ...
        ApplicationManager.getApplication().invokeAndWait {
            val selectedIdx = Messages.showDialog(
                project,
                dialogMessage,
                options["source"]?.let { (it as? Map<*, *>)?.get("label")?.toString() } ?: "arcanea",
    ...
    private fun showNotificationMessage(
        ...
        val notification = NotificationGroupManager.getInstance().getNotificationGroup("arcanea").createNotification(
            message,
            notificationType
        )
