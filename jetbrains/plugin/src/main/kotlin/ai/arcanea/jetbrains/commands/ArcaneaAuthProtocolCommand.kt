...
package ai.arcanea.jetbrains.commands
...
import ai.arcanea.jetbrains.core.PluginContext
...
/**
 * JetBrains Protocol Command for handling Arcanea authentication URLs
 *
 * Handles URLs in the format: jetbrains://idea/ai.arcanea.jetbrains.auth?token=HERE
 * and forwards them to the VSCode extension via RPC protocol
 */
class ArcaneaAuthProtocolCommand : JBProtocolCommand("ai.arcanea.jetbrains.auth") {
    private val logger = Logger.getInstance(ArcaneaAuthProtocolCommand::class.java)
    private val coroutineScope = CoroutineScope(SupervisorJob() + Dispatchers.IO)
    
    companion object {
        const val COMMAND_ID = "ai.arcanea.jetbrains.auth"
    ...
    override suspend fun execute(target: String?, parameters: Map<String, String>, fragment: String?): String? {
        logger.info("Handling Arcanea auth protocol command: target=$target, parameters=$parameters")
        ...
        return try {
            ...
            val errorMsg = "Error handling Arcanea auth protocol command: ${e.message}"
    ...
    private fun forwardTokenToVSCodeExtension(token: String) {
        coroutineScope.launch {
            try {
                ...
                val vscodeUriString = "vscode://arcanea.arcanea/arcanea?token=${token}"
