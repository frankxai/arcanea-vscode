...
import ai.arcanea.jetbrains.commands.CommandRegistry
import ai.arcanea.jetbrains.commands.ICommand
import ai.arcanea.jetbrains.util.URI
import ai.arcanea.jetbrains.util.URIComponents
import java.io.File
    ...
    suspend fun vscode_changes(title: String, changes: List<List<Map<String, Any?>>>): Any? {
        ...
        try {
            ...
            for ((index, changeTriplet) in changes.withIndex()) {
                ...
                val originalFileUri = ai.arcanea.jetbrains.editor.createURI(changeTriplet[0])
                val beforeContentUri = ai.arcanea.jetbrains.editor.createURI(changeTriplet[1])
                val afterContentUri = ai.arcanea.jetbrains.editor.createURI(changeTriplet[2])
