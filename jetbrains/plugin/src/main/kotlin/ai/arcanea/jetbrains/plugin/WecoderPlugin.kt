...
package ai.arcanea.jetbrains.plugin
...
import ai.arcanea.jetbrains.core.ExtensionProcessManager
import ai.arcanea.jetbrains.core.ExtensionSocketServer
import ai.arcanea.jetbrains.core.ServiceProxyRegistry
import ai.arcanea.jetbrains.webview.WebViewManager
import ai.arcanea.jetbrains.workspace.WorkspaceFileChangeManager
...
import ai.arcanea.jetbrains.core.*
import ai.arcanea.jetbrains.util.ExtensionUtils
import ai.arcanea.jetbrains.util.PluginConstants
import ai.arcanea.jetbrains.util.PluginResourceUtil
import java.io.File
...
class WecoderPlugin : StartupActivity.DumbAware {
    ...
    override fun runActivity(project: Project) {
        ...
        LOG.info(
            "Initializing Arcanea plugin for project: ${project.name}, " +
        ...
        try {
            ...
            Disposer.register(project, Disposable {
                LOG.info("Disposing Arcanea plugin for project: ${project.name}")
                pluginService.dispose()
                // SystemObjectProvider is now project-scoped and will be disposed automatically
            })

            LOG.info("Arcanea plugin initialized successfully for project: ${project.name}")
        } catch (e: Exception) {
            LOG.error("Failed to initialize Arcanea plugin", e)
        }
    }
}
...
class WecoderPluginService(private var currentProject: Project) : Disposable {
    ...
    companion object {
        // Debug mode switch
        @Volatile
        private var DEBUG_TYPE: DEBUG_MODE = ai.arcanea.jetbrains.plugin.DEBUG_MODE.NONE
        ...
        init {
            try {
                ...
                val configStream: InputStream? = WecoderPluginService::class.java.getResourceAsStream("/ai/arcanea/jetbrains/plugin/config/plugin.properties")
...
//                ServiceProxyRegistry.getInstance().initialize()
                ...
                if (DEBUG_TYPE == ai.arcanea.jetbrains.plugin.DEBUG_MODE.ALL) {
    ...
    private fun cleanup() {
        ...
        try {
            if (DEBUG_TYPE == ai.arcanea.jetbrains.plugin.DEBUG_MODE.NONE) {
