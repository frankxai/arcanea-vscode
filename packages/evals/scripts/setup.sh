...
build_extension() {
  echo "🔨 Building the Arcanea extension..."
  pnpm -w vsix -- --out ../bin/arcanea-$(git rev-parse --short HEAD).vsix || exit 1
  code --install-extension ../../bin/arcanea-$(git rev-parse --short HEAD).vsix || exit 1
  cd evals
}
...
if ! code --list-extensions 2>/dev/null | grep -q "arcanea.arcanea"; then
  code --install-extension arcanea.arcanea &>/dev/null || exit 1
fi
...
echo -n "🗄️ Syncing Arcanea evals database... "
...
current_version=$(code --list-extensions --show-versions 2>/dev/null | grep arcanea)
read -p "💻 Do you want to build a new version of the Arcanea extension? [currently $current_version] (y/N): " build_extension
