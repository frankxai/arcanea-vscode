...
describe("PostHogTelemetryClient", () => {
	...
	describe("isEventCapturable", () => {
		...
		it("should return false for events in exclude list", () => {
			...
			expect(isEventCapturable(TelemetryEventName.TASK_MESSAGE /*arcanea_change*/)).toBe(false)
		})
	})
	...
	describe("capture", () => {
		...
		it("should not capture events that are not capturable", async () => {
			...
			await client.capture({
				event: TelemetryEventName.TASK_MESSAGE, // This is in the exclude list. // arcanea_change
				properties: { test: "value" },
			})
