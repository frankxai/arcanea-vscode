...
describe("DEFAULT_HEADERS", () => {
	...
	it("should have correct HTTP-Referer value", () => {
		expect(DEFAULT_HEADERS["HTTP-Referer"]).toBe("https://arcanea.ai")
	})

	it("should have correct X-Title value", () => {
		expect(DEFAULT_HEADERS["X-Title"]).toBe("Arcanea")
	})

	it("should have correct User-Agent format", () => {
		const userAgent = DEFAULT_HEADERS["User-Agent"]
		expect(userAgent).toBe(`Arcanea-Code/${Package.version}`)
	...
	it("should have User-Agent with correct tool name", () => {
		const userAgent = DEFAULT_HEADERS["User-Agent"]
		expect(userAgent.startsWith("Arcanea-Code/")).toBe(true)
	})
	...
	it("should have exactly 4 headers", () => {
		...
		expect(headerKeys).toEqual(["HTTP-Referer", "X-Title", "X-Arcanea-Version", "User-Agent"])
	})
})
