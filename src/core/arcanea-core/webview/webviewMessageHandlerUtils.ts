...
const resendMessageSequence = async (
	...
	if (!currentCline || currentCline.taskId !== taskId) {
		...
		vscode.window.showErrorMessage(t("arcanea:userFeedback.message_update_failed"))
		return false
	}
...
export const fetchArcaneacodeNotificationsHandler = async (provider: ClineProvider) => {
	try {
		const { apiConfiguration } = await provider.getState()
		const arcaneaToken = apiConfiguration?.arcaneaToken

		if (!arcaneaToken || apiConfiguration?.apiProvider !== "arcanea") {
		...
		const response = await axios.get(`${getArcaneaBaseUriFromToken(arcaneaToken)}/api/users/notifications`, {
			headers,
			timeout: 5000,
		})
		...
		provider.log(`Error fetching Arcaneacode notifications: ${error.message}`)
		provider.postMessageToWebview({
			type: "arcaneaNotificationsResponse",
			notifications: [],
		})
	...
	try {
		...
		if (revert) {
			...
			if (checkpointMessage && checkpointMessage.text) {
				...
				vscode.window.showErrorMessage(t("arcanea:userFeedback.no_checkpoint_found"))
			}
		}
		...
		if (success) {
			vscode.window.showInformationMessage(t("arcanea:userFeedback.message_updated"))
		}
	} catch (error) {
