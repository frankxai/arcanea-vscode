...
import { CommitRange } from "@arcanea/types"
...
export async function seeNewChanges(task: Task, commitRange: CommitRange) {
	try {
		const service = await getCheckpointService(task)
		if (!service) {
			vscode.window.showWarningMessage(t("arcanea:seeNewChanges.checkpointsUnavailable"))
			return
		}

		const changes = await service.getDiff(commitRange)
		if (changes.length === 0) {
			vscode.window.showWarningMessage(t("arcanea:seeNewChanges.noChanges"))
			return
		}

		await vscode.commands.executeCommand(
			"vscode.changes",
			t("arcanea:seeNewChanges.title"),
			changes.map((change) => [
		...
		vscode.window.showErrorMessage(t("arcanea:seeNewChanges.error"))
