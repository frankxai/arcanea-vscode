...
describe("CustomModesManager", () => {
	...
	const mockRoomodes = path.join(mockWorkspacePath, ".arcaneamodes")
	...
	describe("getCustomModes", () => {
		...
		it("should merge modes with .arcaneamodes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .arcaneamodes since it takes precedence
		...
		it("should handle missing .arcaneamodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .arcaneamodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .arcaneamodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .arcaneamodes precedence", async () => {
			...
			// Should update global state with merged modes where .arcaneamodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .arcaneamodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .arcaneamodes file when adding project-specific mode", async () => {
			...
			// Mock .arcaneamodes to not exist initially
			...
			// Verify .arcaneamodes was created with the project mode
			...
			// Verify the content written to .arcaneamodes
	...
	describe("updateModesInFile", () => {
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".arcaneamodes"),
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".arcaneamodes"))
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should work with global custom modes when .roomodes doesn't exist", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.arcanea/rules-test-mode" },
			])
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a built-in mode customized in .roomodes", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.arcanea/rules-code" },
			])
