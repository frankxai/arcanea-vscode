...
describe("RooIgnore Response Formatting", () => {
	...
	describe("formatResponse.rooIgnoreError", () => {
		...
		it("should format error message for ignored files", () => {
			...
			expect(errorMessage).toContain(
				"Access to secrets/api-keys.json is blocked by the .arcaneaignore file settings",
			)
			expect(errorMessage).toContain("continue in the task without using this file")
			expect(errorMessage).toContain("ask the user to update the .arcaneaignore file")
		})
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .arcaneaignore instructions for the LLM", async () => {
			// Create controller
			const controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .arcaneaignore exists", async () => {
			// Set up no .arcaneaignore
			mockFileExists.mockResolvedValue(false)

			// Create controller without .arcaneaignore
			const controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
