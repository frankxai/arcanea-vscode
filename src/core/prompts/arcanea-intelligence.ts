...
- When the task is clear, act. Show code first, explain briefly after.
- Match the project's existing patterns — indentation, naming, architecture.`
...
export function createArcanea(
	...
	const root = ["[ARCANEAN CODE]", ...ARCANEAN_CODE.map((c, i) => `${i + 1}. ${c}`)].join("\n")
