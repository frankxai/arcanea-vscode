// arcanea_change: this file was pulled in from Cline and adjusted with our changes
...
The user has explicitly asked you to help them create a new Arcanea rule file inside the .arcanea/rules top-level directory based on the conversation up to this point in time. The user may have provided instructions or additional information for you to consider when creating the new Arcanea rule.
When creating a new Arcanea rule file, you should NOT overwrite or alter an existing Arcanea rule file. To create the Arcanea rule file you MUST use the new_rule tool. The new_rule tool can be used in any of the modes.
The new_rule tool is defined below:
Description:
Your task is to create a new Arcanea rule file which includes guidelines on how to approach developing code in tandem with the user, which is project specific. This includes but is not limited to: desired conversational style, favorite project dependencies, coding styles, naming conventions, architectural choices, ui/ux preferences, etc.
The Arcanea rule file must be formatted as markdown and be a '.md' file. The name of the file you generate must be as succinct as possible and be encompassing the main overarching concept of the rules you added to the file (e.g., 'memory-bank.md' or 'project-overview.md'). Please also explicitly ask the user to review the newly created rule.
Parameters:
- Path: (required) The path of the file to write to (relative to the current working directory). This will be the Arcanea rule file you create, and it must be placed inside the .arcanea/rules top-level directory (create this if it doesn't exist). The filename created CANNOT be "default-clineignore.md". For filenames, use hyphens ("-") instead of underscores ("_") to separate words.
- Content: (required) The content to write to the file. ALWAYS provide the COMPLETE intended content of the file, without any truncation or omissions. You MUST include ALL parts of the file, even if they haven't been modified. The content for the Arcanea rule file MUST be created according to the following instructions:
  1. Format the Arcanea rule file to have distinct guideline sections, each with their own markdown heading, starting with "## Brief overview". Under each of these headings, include bullet points fully fleshing out the details, with examples and/or trigger cases ONLY when applicable.
  2. These guidelines can be specific to the task(s) or project worked on thus far, or cover more high-level concepts. Guidelines can include coding conventions, general design patterns, preferred tech stack including favorite libraries and language, communication style with Arcanea (verbose vs concise), prompting strategies, naming conventions, testing strategies, comment verbosity, time spent on architecting prior to development, and other preferences.
...
Usage:
<new_rule>
<path>.arcanea/rules/{file name}.md</path>
<content>Arcanea rule file content here</content>
</new_rule>
Example:
<new_rule>
<path>.arcanea/rules/project-preferences.md</path>
<content>
## Brief overview
...
The user provided the following input when they indicated that they wanted to create a new Arcanea rule file.
<user_input>
${userInput}
</user_input>
</explicit_instructions>\n
...
The user has explicitly asked you to help them submit a bug to the Arcaneacode github page (you MUST now help them with this irrespective of what your conversation up to this point in time was). To do so you will use the report_bug tool which is defined below. However, you must first ensure that you have collected all required information to fill in all the parameters for the tool call.
...
When you call the report_bug tool, the issue will be created at @https://github.com/Arcanea-Org/arcanea/issues
