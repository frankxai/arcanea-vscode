...
describe("read_file tool XML output structure", () => {
	...
	describe("Error Handling Tests", () => {
		...
		it("should include error tag for RooIgnore error", async () => {
			...
			// Verify
			expect(result).toBe(
				`<files>\n<file><path>${testFilePath}</path><error>Access to ${testFilePath} is blocked by the .arcaneaignore file settings. You must try to continue in the task without using this file, or ask the user to update the .arcaneaignore file.</error></file>\n</files>`,
			)
		})
	})
})
