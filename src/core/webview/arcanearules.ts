...
import { allowedExtensions } from "../../shared/arcanea/rules"
...
function getRuleDirectoryPath(baseDir: string, ruleType: "rule" | "workflow") {
	return ruleType === "workflow"
		...
		: path.join(baseDir, GlobalFileNames.arcaneaRules)
}
...
export async function createRuleFile(
	...
	if (!workspacePath && !isGlobal) {
		vscode.window.showErrorMessage(t("arcanea:rules.errors.noWorkspaceFound"))
		return
	}
	...
	if (await fileExistsAtPath(filePath)) {
		vscode.window.showErrorMessage(t("arcanea:rules.errors.fileAlreadyExists", { filename }))
		return
	}
...
${t("arcanea:rules.templates.workflow.description")}

${t("arcanea:rules.templates.workflow.stepsHeader")}

1. ${t("arcanea:rules.templates.workflow.step1")}
2. ${t("arcanea:rules.templates.workflow.step2")}
`
}
...
${t("arcanea:rules.templates.rule.description")}

${t("arcanea:rules.templates.rule.guidelinesHeader")}

- ${t("arcanea:rules.templates.rule.guideline1")}
- ${t("arcanea:rules.templates.rule.guideline2")}
`
}

export async function deleteRuleFile(rulePath: string): Promise<void> {
	const deleteAction = t("arcanea:rules.actions.delete")
	const filename = path.basename(rulePath)
	const result = await vscode.window.showWarningMessage(
		t("arcanea:rules.actions.confirmDelete", { filename }),
		{ modal: true },
		deleteAction,
	)
	...
	if (result === deleteAction) {
		...
		vscode.window.showInformationMessage(t("arcanea:rules.actions.deleted", { filename }))
	}
}
