...
async function showMacOSNotification(options: NotificationOptions): Promise<void> {
	...
	try {
		...
		// Add Arcanea logo
		const extensionUri = vscode.extensions.getExtension(`arcanea.arcanea`)!.extensionUri
		const iconPath = vscode.Uri.joinPath(extensionUri, "assets", "icons", "arcanea.png").fsPath
...
"@
    ...
    [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Arcanea").Show($toast)
    `
...
export async function showSystemNotification(options: NotificationOptions): Promise<void> {
	try {
		const { title = "Arcanea", message } = options
