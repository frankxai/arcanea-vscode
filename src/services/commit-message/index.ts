...
export function registerCommitMessageProvider(
	...
	commitProvider.activate().catch((error) => {
		outputChannel.appendLine(t("arcanea:commitMessage.activationFailed", { error: error.message }))
		console.error("Commit message provider activation failed:", error)
	})

	outputChannel.appendLine(t("arcanea:commitMessage.providerRegistered"))
}
