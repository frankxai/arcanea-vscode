...
export class GhostDecorationAnimation {
	...
	private readonly animationFrames = ["█", "A█", "A█", "AR█", "ARC█"]
	...
	private updateDecorationText(): void {
		...
		if (this.animationState === this.animationFrames.length - 1) {
			// Show either the full frame with block, or just "ARC" without block
			text = this.isBlockVisible ? this.animationFrames[this.animationState] : "ARC"
		} else {
