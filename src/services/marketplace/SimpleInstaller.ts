...
export class SimpleInstaller {
	...
	private async installMode(
		...
		try {
			...
			if (error.code === "ENOENT") {
				...
				const fileName = target === "project" ? ".arcaneamodes" : "custom-modes.yaml"
				throw new Error(
	...
	private async installMcp(
		...
		try {
			...
			if (error.code === "ENOENT") {
				...
				const fileName = target === "project" ? ".arcanea/mcp.json" : "mcp-settings.json"
				throw new Error(
	...
	private async getModeFilePath(target: "project" | "global"): Promise<string> {
		if (target === "project") {
			...
			return path.join(workspaceFolder.uri.fsPath, ".arcaneamodes")
		} else {
