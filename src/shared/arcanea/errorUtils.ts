...
export function isModelNotAllowedForTeamError(error: any) {
	//https://github.com/Arcanea-Org/arcanea-backend/blob/66489e1b0f6f996338acf8bcc1b3558252e20e9d/src/app/api/openrouter/%5B...path%5D/route.ts#L371
	return !!(error && error.status === 404 && (error.message?.indexOf("not allowed for your team") ?? -1) >= 0)
}

export function isAnyRecognizedArcaneaError(error: any) {
	return (
