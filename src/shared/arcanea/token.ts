export function getArcaneaBaseUriFromToken(arcaneaToken?: string) {
	if (arcaneaToken) {
		try {
			const payload_string = arcaneaToken.split(".")[1]
			const payload_json =
			...
			console.warn("Failed to get base URL from Arcanea token")
		}
	}
	return "https://api.arcanea.ai"
}
