...
import { isAlphaPeriodEndedError, isModelNotAllowedForTeamError } from "@roo/arcanea/errorUtils"
...
function warningText(innerMessage?: InnerMessage) {
	...
	if (isAlphaPeriodEndedError(innerMessage?.error)) {
		return i18next.t("arcanea:invalidModel.alphaPeriodEnded", { model: unavailableModel })
	}
	if (isModelNotAllowedForTeamError(innerMessage?.error)) {
		return i18next.t("arcanea:invalidModel.notAllowedForTeam", { model: unavailableModel })
	}
	return i18next.t("arcanea:invalidModel.modelUnavailable", { model: unavailableModel })
}
				...
				<>
					<VSCodeButton
						...
						{canChangeToDefaultModel
							? t("arcanea:invalidModel.continueWith", {
									model: providerModels[defaultModelId]?.displayName ?? defaultModelId,
								})
							: t("arcanea:invalidModel.continue")}
					</VSCodeButton>
