...
export const OrganizationSelector = ({ className, showLabel = false }: { className?: string; showLabel?: boolean }) => {
	...
	const selectedOrg = organizations.find((o) => o.id === apiConfiguration?.arcaneaOrganizationId)
	...
	useEffect(() => {
		if (!apiConfiguration?.arcaneaToken) return
	...
	const setSelectedOrganization = (organization: UserOrganizationWithApiKey | null) => {
		if (organization === null) {
			...
			vscode.postMessage({
				...
				values: {
					apiKey: apiConfiguration?.arcaneaToken,
				},
			})
		} else {
			vscode.postMessage({
				...
				apiConfiguration: {
					...apiConfiguration,
					arcaneaOrganizationId: organization.id,
				},
			})
			vscode.postMessage({
				type: "fetchBalanceDataRequest",
	...
	return (
		<div className={className}>
			{showLabel && (
				<div>
					<label className="block font-medium mb-2">{t("arcanea:profile.organization")}</label>
				</div>
			)}
			<div className="relative" ref={containerRef}>
				<button
					...
					<span className="truncate">{selectedOrg ? selectedOrg.name : t("arcanea:profile.personal")}</span>
				...
				{isOpen && (
					<div className="absolute z-20 mt-1 right-0 w-max min-w-full max-h-60 overflow-auto rounded border border-[var(--vscode-dropdown-border)] bg-[var(--vscode-dropdown-background)] shadow">
						<div role="listbox" aria-label={t("arcanea:profile.organization")}>
							<button
								type="button"
								role="option"
								...
								<span className="truncate">{t("arcanea:profile.personal")}</span>
							</button>
