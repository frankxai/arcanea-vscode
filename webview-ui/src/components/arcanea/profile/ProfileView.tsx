...
import ArcaneaAuth from "../common/ArcaneaAuth"
...
const ProfileView: React.FC<ProfileViewProps> = ({ onDone }) => {
	...
	}, [apiConfiguration?.arcaneaToken, apiConfiguration?.arcaneaOrganizationId])
	...
	return (
		<Tab>
			<TabHeader className="flex justify-between items-center">
				<h3 className="text-vscode-foreground m-0">{t("arcanea:profile.title")}</h3>
							...
							<div className="flex flex-col pr-3 h-full">
								...
								<div className="w-full flex gap-2 flex-col min-[225px]:flex-row">
									<div className="w-full min-[225px]:w-1/2">
										<VSCodeButtonLink
											href="https://arcanea.ai/profile"
											appearance="primary"
											className="w-full">
											{t("arcanea:profile.dashboard")}
										</VSCodeButtonLink>
									</div>
									<VSCodeButton
								...
								<div className="w-full flex flex-col items-center">
									<div className="text-sm text-[var(--vscode-descriptionForeground)] mb-3">
										{t("arcanea:profile.currentBalance")}
									</div>

									<div className="text-4xl font-bold text-[var(--vscode-foreground)] mb-6 flex items-center gap-2">
										{isLoadingBalance ? (
											<div className="text-[var(--vscode-descriptionForeground)]">
												{t("arcanea:profile.loading")}
											</div>
										) : (
											balance && (
												<>
										...
										<div className="w-full mt-8">
														...
														{pkg.popular && (
															<div className="absolute -top-2 left-1/2 transform -translate-x-1/2">
																<span className="bg-[var(--vscode-button-background)] text-[var(--vscode-button-foreground)] text-xs px-2 py-1 rounded-full font-medium">
																	{t("arcanea:profile.shop.popular")}
																</span>
															</div>
														)}
											...
											<div className="text-center">
												<VSCodeButtonLink
													...
													appearance="secondary"
													className="text-sm">
													{t("arcanea:profile.shop.viewAll")}
												</VSCodeButtonLink>
											</div>
										</div>
									)}
