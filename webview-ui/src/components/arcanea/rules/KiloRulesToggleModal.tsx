...
const ArcaneaRulesToggleModal: React.FC = () => {
					...
					<Tooltip open={isVisible ? false : undefined}>
						<TooltipTrigger asChild>
							<BottomButton
								...
								ariaLabel={t("arcanea:rules.ariaLabel")}
								onClick={() => setIsVisible(!isVisible)}
							/>
						</TooltipTrigger>
						<TooltipContent>{t("arcanea:rules.tooltip")}</TooltipContent>
					</Tooltip>
				</TooltipProvider>
			</div>
			...
			{isVisible && (
				<div
					...
					<div
						...
						<div
							...
							<StyledTabButton $isActive={currentView === "rule"} onClick={() => setCurrentView("rule")}>
								{t("arcanea:rules.tabs.rules")}
							</StyledTabButton>
							<StyledTabButton
								$isActive={currentView === "workflow"}
								onClick={() => setCurrentView("workflow")}>
								{t("arcanea:rules.tabs.workflows")}
							</StyledTabButton>
						</div>
					</div>
					...
					<div className="text-xs text-[var(--vscode-descriptionForeground)] mb-4">
						{currentView === "rule" ? (
							<DescriptionWithLink
								href="https://arcanea.ai/docs/advanced-usage/custom-rules"
								linkText={t("arcanea:docs")}>
								{t("arcanea:rules.description.rules")}
							</DescriptionWithLink>
						) : (
							<DescriptionWithLink
								href="https://arcanea.ai/docs/features/slash-commands/workflows"
								linkText={t("arcanea:docs")}>
								{t("arcanea:rules.description.workflows")}{" "}
								<span className="text-[var(--vscode-foreground)] font-bold">/workflow-name</span>{" "}
								{t("arcanea:rules.description.workflowsInChat")}
							</DescriptionWithLink>
						)}
					</div>
...
export default ArcaneaRulesToggleModal
