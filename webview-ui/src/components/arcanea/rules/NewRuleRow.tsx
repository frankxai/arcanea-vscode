...
import { getExtension } from "@/utils/arcanea/path-webview"
import { allowedExtensions } from "@roo/arcanea/rules"
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		if (filename.trim()) {
			...
			if (extension !== "" && !allowedExtensions.includes(extension)) {
				setError(t("arcanea:rules.validation.invalidFileExtension"))
				return
			}
				...
				{isExpanded ? (
					<form onSubmit={handleSubmit} className="flex flex-1 items-center">
						<input
							...
							placeholder={
								ruleType === "workflow"
									? t("arcanea:rules.placeholders.workflowName")
									: t("arcanea:rules.placeholders.ruleName")
							}
							value={filename}
					...
					<>
						<span className="flex-1 text-[var(--vscode-descriptionForeground)] bg-[var(--vscode-input-background)] italic text-xs">
							{ruleType === "workflow"
								? t("arcanea:rules.newFile.newWorkflowFile")
								: t("arcanea:rules.newFile.newRuleFile")}
						</span>
						<div className="flex items-center ml-2 space-x-2">
							<VSCodeButton
								appearance="icon"
