...
export const GhostServiceSettingsView = ({
	...
	const keybindings = useKeybindings(["arcanea.ghost.promptCodeSuggestion", "arcanea.ghost.generateSuggestions"])
				...
				<div className="flex flex-col gap-3">
					<div className="flex flex-col gap-1">
						<div className="flex items-center gap-2 font-bold">
							<Zap className="w-4" />
							<div>{t("arcanea:ghost.settings.triggers")}</div>
						</div>
					</div>

					<div className="flex flex-col gap-1">
						<ControlledCheckbox checked={enableAutoTrigger || false} onChange={onEnableAutoTriggerChange}>
							<span className="font-medium">{t("arcanea:ghost.settings.enableAutoTrigger.label")}</span>
						</ControlledCheckbox>
						<div className="text-vscode-descriptionForeground text-sm mt-1">
							<Trans i18nKey="arcanea:ghost.settings.enableAutoTrigger.description" />
						</div>
					</div>
					...
					{enableAutoTrigger && (
						<div className="flex flex-col gap-1">
							<label className="block font-medium text-sm">
								{t("arcanea:ghost.settings.autoTriggerDelay.label")}
							</label>
							...
							<div className="text-vscode-descriptionForeground text-xs mt-1">
								<Trans i18nKey="arcanea:ghost.settings.autoTriggerDelay.description" />
							</div>
						</div>
					)}
					...
					<div className="flex flex-col gap-1">
						<ControlledCheckbox
							...
							<span className="font-medium">
								{t("arcanea:ghost.settings.enableQuickInlineTaskKeybinding.label", {
									keybinding: keybindings["arcanea.ghost.promptCodeSuggestion"],
								})}
							</span>
						</ControlledCheckbox>
						<div className="text-vscode-descriptionForeground text-sm mt-1">
							<Trans
								i18nKey="arcanea:ghost.settings.enableQuickInlineTaskKeybinding.description"
								components={{
									DocsLink: (
										<a
											href="#"
											onClick={() =>
												openGlobalKeybindings("arcanea.ghost.promptCodeSuggestion")
											}
					...
					<div className="flex flex-col gap-1">
						<ControlledCheckbox
							...
							<span className="font-medium">
								{t("arcanea:ghost.settings.enableSmartInlineTaskKeybinding.label", {
									keybinding: keybindings["arcanea.ghost.generateSuggestions"],
								})}
							</span>
						</ControlledCheckbox>
						<div className="text-vscode-descriptionForeground text-sm mt-1">
							<Trans
								i18nKey="arcanea:ghost.settings.enableSmartInlineTaskKeybinding.description"
								values={{ keybinding: keybindings["arcanea.ghost.generateSuggestions"] }}
								components={{
									DocsLink: (
										<a
											href="#"
											onClick={() => openGlobalKeybindings("arcanea.ghost.generateSuggestions")}
											className="text-[var(--vscode-list-highlightForeground)] hover:underline cursor-pointer"></a>
									),
								}}
							/>
						...
						<div className="flex flex-col gap-3">
							<div className="flex flex-col gap-1">
								<div className="flex items-center gap-2 font-bold">
									...
									<div>{t("arcanea:ghost.settings.provider")}</div>
								</div>
							</div>
							<div className="flex flex-col gap-1">
								<ControlledCheckbox
									...
									<span className="font-medium">
										{t("arcanea:ghost.settings.enableCustomProvider.label")}
									</span>
								</ControlledCheckbox>
								<div className="text-vscode-descriptionForeground text-sm mt-1">
									<Trans i18nKey="arcanea:ghost.settings.enableCustomProvider.description" />
								</div>
							</div>
							{enableCustomProvider && (
								<div className="flex flex-col gap-3">
									<div>
										<label className="block font-medium mb-1">
											{t("arcanea:ghost.settings.apiConfigId.label")}
										</label>
										<div className="flex items-center gap-2">
											<div>
												<Select value={apiConfigId || "-"} onValueChange={onApiConfigIdChange}>
													<SelectTrigger
														...
														<SelectValue
															placeholder={t(
																"arcanea:ghost.settings.apiConfigId.current",
															)}
														/>
													</SelectTrigger>
													<SelectContent>
														<SelectItem value="-">
															{t("arcanea:ghost.settings.apiConfigId.current")}
														</SelectItem>
												...
												<div className="text-sm text-vscode-descriptionForeground mt-1">
													{t("arcanea:ghost.settings.apiConfigId.description")}
												</div>
											</div>
										</div>
									</div>
