import { type ModelInfo, type ProviderSettings } from "@arcanea/types"
...
import { getArcaneaBaseUriFromToken } from "@roo/arcanea/token"
			...
			<table className="w-full text-sm">
				<thead>
					<tr className="border-b border-vscode-widget-border bg-vscode-editor-background">
						<th className={thClass}>{t("arcanea:settings.modelInfo.table.provider")}</th>
						<StandardTooltip content={t("arcanea:settings.modelInfo.table.context")}>
							<th className={thClass}>Context</th>
						</StandardTooltip>
						<StandardTooltip content={t("arcanea:settings.modelInfo.table.inputPrice")}>
							<th className={thClass}>Input</th>
						</StandardTooltip>
						<StandardTooltip content={t("arcanea:settings.modelInfo.table.outputPrice")}>
							<th className={thClass}>Output</th>
						</StandardTooltip>
						<StandardTooltip content={t("arcanea:settings.modelInfo.table.cacheReadPrice")}>
							<th className={thClass}>C Read</th>
						</StandardTooltip>
						<StandardTooltip content={t("arcanea:settings.modelInfo.table.cacheWritePrice")}>
...
export const ArcaneaModelInfoView = ({
	apiConfiguration,
	modelId,
	model,
	...
	const { data: modelStats } = useQuery<ModelStats[]>({
		...
		queryFn: async () => {
			try {
				return (
					await fetch(`${getArcaneaBaseUriFromToken(apiConfiguration.arcaneaToken ?? "")}/api/modelstats`)
				).json()
			} catch (err) {
	...
	return (
		<>
			...
			{apiConfiguration.apiProvider === "arcanea" && modelId.endsWith(":free") && (
			...
			<div className="text-sm text-vscode-descriptionForeground">
				...
				/>
				<div>
					<span className="font-medium">{t("arcanea:settings.modelInfo.contextWindow")}:</span>{" "}
			...
			{stats && stats.cost && stats.costPerRequest && model.inputPrice && model.outputPrice ? (
				<StandardTooltip content={t("arcanea:settings.modelInfo.averageArcaneaCost")}>
			...
			<Collapsible open={isPricingExpanded} onOpenChange={setIsPricingExpanded}>
				<CollapsibleTrigger className="flex items-center gap-1 w-full cursor-pointer hover:opacity-80 mb-2">
					...
					<span className="font-medium">{t("arcanea:settings.modelInfo.providerBreakdown")}</span>
				</CollapsibleTrigger>
