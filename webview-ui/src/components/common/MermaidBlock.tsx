...
import { MermaidSyntaxFixer } from "@src/services/mermaidSyntaxFixer" // arcanea_change
import CodeBlock from "../arcanea/common/CodeBlock" // arcanea_change
import { MermaidButton } from "@/components/common/MermaidButton"
import { MermaidFixButton } from "@/components/common/MermaidFixButton" // arcanea_change
...
// arcanea_change next line rename to originalCode to keep the difference with Roo smaller
export default function MermaidBlock({ code: originalCode }: MermaidBlockProps) {
	...
	useEffect(() => {
		...
		setIsFixing(false)
		// arcanea_change end
	}, [originalCode]) // arcanea_change originalCode instead of code
	...
	useDebounceEffect(
		...
		500, // Delay 500ms
		[code, isFixing, originalCode, t], // Dependencies for scheduling // arcanea_change added isFixing, originalCode and t
	)
	...
	return (
		<MermaidBlockContainer>
			{isLoading && (
				<LoadingMessage>
					{isFixing /* arcanea_change */ ? t("common:mermaid.fixing_syntax") : t("common:mermaid.loading")}
				</LoadingMessage>
			)}
			...
			{error ? (
				...
				<MermaidButton containerRef={containerRef} code={code} isLoading={isLoading} svgToPng={svgToPng}>
					{/* arcanea_change start switched from ref to dangerouslySetInnerHTML */}
					<SvgContainer
						onClick={handleClick}
						ref={containerRef}
						$isLoading={isLoading}
