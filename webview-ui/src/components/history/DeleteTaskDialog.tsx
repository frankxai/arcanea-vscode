...
export const DeleteTaskDialog = ({ taskId, ...props }: DeleteTaskDialogProps) => {
	...
	const { data: tasks } = useTaskWithId([taskId]) // arcanea_change

	const { onOpenChange } = props

	const task = tasks?.find((t) => t.id === taskId) // arcanea_change
	const isFavorited = task?.isFavorited // arcanea_change
