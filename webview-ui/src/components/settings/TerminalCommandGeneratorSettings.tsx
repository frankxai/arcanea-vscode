		...
		<div className={cn("flex flex-col gap-3", className)} {...props}>
			<div className="flex flex-col gap-1">
				<div className="flex items-center gap-2 font-bold">
					...
					<div>{t("arcanea:settings.terminal.commandGenerator.provider")}</div>
				</div>
			</div>
			<div className="flex flex-col gap-3 pl-3 border-l-2 border-vscode-button-background">
				<div>
					<label className="block font-medium mb-1">
						{t("arcanea:settings.terminal.commandGenerator.apiConfigId.label")}
					</label>
						...
						<div>
							<Select
								...
								<SelectTrigger data-testid="terminal-command-api-config-select" className="w-full">
									<SelectValue
										placeholder={t(
											"arcanea:settings.terminal.commandGenerator.apiConfigId.current",
										)}
									/>
								</SelectTrigger>
								<SelectContent>
									<SelectItem value="-">
										{t("arcanea:settings.terminal.commandGenerator.apiConfigId.current")}
									</SelectItem>
							...
							<div className="text-sm text-vscode-descriptionForeground mt-1">
								{t("arcanea:settings.terminal.commandGenerator.apiConfigId.description")}
							</div>
						</div>
					</div>
				</div>
