 ...
 */
// arcanea_change: unused campaign param
export function buildDocLink(path: string, _campaign: string): string {
	// Remove any leading slash from path
	const cleanPath = path
		.replace(/^\//, "")
		.replace("troubleshooting/shell-integration/", "features/shell-integration") // arcanea_change
	const [basePath, hash] = cleanPath.split("#")
	const baseUrl = `https://arcanea.ai/docs/${basePath}`
